...
class ArticleBuilder {
    ...
    async handleImageUpload(blockId, file) {
        if (!file) {
    ...
    selectImageFile(blockId) {
        ...
        input.onchange = (e) => {
            if (e.target.files && e.target.files[0]) {
                this.handleImageUpload(blockId, e.target.files[0]);
            }
        };
        input.click();
    }
    ...
    render() {
        ...
        if (this.blocks.length === 0) {
            grid.innerHTML = `
                <div style="grid-column: span 2; text-align: center; padding: 60px; color: #6b7280; background: #f9fafb; border-radius: 12px; border: 2px dashed #e5e7eb;">
                    ...
                    <p>Нет блоков. Нажмите "Текстовый блок" или "Блок изображения"</p>
                </div>
            `;
            return;
        }
        ...
        this.blocks.forEach((block, index) => {
            ...
            blockDiv.style.borderRadius = '20px';
            ...
            // Стеклянная рамка для текстовых блоков
            ...
            dragHandle.onmouseenter = () => dragHandle.style.background = '#d1d5db';
            dragHandle.onmouseleave = () => dragHandle.style.background = '#e5e7eb';
            ...
            if (block.type === 'text') {
                ...
                // Glass frame checkbox
                const glassCheckbox = document.createElement('div');
                glassCheckbox.style.cssText = 'background: #f3f4f6; padding: 6px 10px; border-radius: 20px; font-size: 12px; display: flex; align-items: center; gap: 6px; cursor: pointer; transition: all 0.2s;';
                glassCheckbox.innerHTML = `
                    <input type="checkbox" id="glass-${block.id}" ${block.glassFrame ? 'checked' : ''} style="margin: 0; cursor: pointer;">
                    <label for="glass-${block.id}" style="cursor: pointer; font-size: 11px;">✨ Стекло</label>
                `;
                const glassCheckboxInput = glassCheckbox.querySelector('input');
                glassCheckboxInput.onchange = (e) => {
                    e.stopPropagation();
                ...
                wideToggle.style.cssText = 'background: #f3f4f6; padding: 6px 10px; border-radius: 20px; font-size: 12px; display: flex; align-items: center; gap: 6px; cursor: pointer; transition: all 0.2s;';
                wideToggle.innerHTML = `
                    <input type="checkbox" id="wide-${block.id}" ${block.wide ? 'checked' : ''} style="margin: 0; cursor: pointer;">
                    <label for="wide-${block.id}" style="cursor: pointer; font-size: 11px;">📐 На всю ширину</label>
                `;
                const wideToggleInput = wideToggle.querySelector('input');
                wideToggleInput.onchange = (e) => {
                    e.stopPropagation();
                ...
                toolbar.innerHTML = `
                    <button type="button" data-cmd="bold" style="padding: 6px 14px; border: 1px solid #d1d5db; background: white; border-radius: 8px; cursor: pointer; transition: all 0.2s;"><b>B</b></button>
                    <button type="button" data-cmd="italic" style="padding: 6px 14px; border: 1px solid #d1d5db; background: white; border-radius: 8px; cursor: pointer; transition: all 0.2s;"><i>I</i></button>
                    <button type="button" data-cmd="underline" style="padding: 6px 14px; border: 1px solid #d1d5db; background: white; border-radius: 8px; cursor: pointer; transition: all 0.2s;"><u>U</u></button>
                    <button type="button" data-cmd="h1" style="padding: 6px 14px; border: 1px solid #d1d5db; background: white; border-radius: 8px; cursor: pointer; transition: all 0.2s;">H1</button>
                    <button type="button" data-cmd="h2" style="padding: 6px 14px; border: 1px solid #d1d5db; background: white; border-radius: 8px; cursor: pointer; transition: all 0.2s;">H2</button>
                    <button type="button" data-cmd="h3" style="padding: 6px 14px; border: 1px solid #d1d5db; background: white; border-radius: 8px; cursor: pointer; transition: all 0.2s;">H3</button>
                `;
                ...
                // Wide toggle button для изображений
                const wideToggle = document.createElement('div');
                wideToggle.style.cssText = 'background: rgba(0,0,0,0.6); padding: 6px 10px; border-radius: 20px; font-size: 12px; display: flex; align-items: center; gap: 6px; cursor: pointer; transition: all 0.2s; color: white;';
                wideToggle.innerHTML = `
                    <input type="checkbox" id="wide-img-${block.id}" ${block.wide ? 'checked' : ''} style="margin: 0; cursor: pointer;">
                    <label for="wide-img-${block.id}" style="cursor: pointer; font-size: 11px;">📐 На всю ширину</label>
                `;
                const wideToggleInput = wideToggle.querySelector('input');
                wideToggleInput.onchange = (e) => {
                    e.stopPropagation();
                ...
                imageContainer.style.cssText = 'text-align: center; min-height: 200px; display: flex; align-items: center; justify-content: center; background: transparent; border-radius: 16px; position: relative; flex-direction: column;';
                
                imageContainer.ondragover = (e) => this.handleDragOver(e);
                imageContainer.ondrop = (e) => this.handleDrop(e, block.id);
                
                if (block.imageUrl && block.imageUrl !== 'null' && block.imageUrl !== 'undefined') {
                    const img = document.createElement('img');
                    img.src = block.imageUrl;
                    img.style.cssText = 'width: 100%; height: auto; border-radius: 16px; display: block; background: transparent;';
                    ...
                    changeBtn.style.cssText = 'background: rgba(0,0,0,0.7); color: white; border: none; border-radius: 8px; padding: 6px 12px; cursor: pointer; font-size: 12px; transition: all 0.2s;';
                    changeBtn.onmouseenter = () => changeBtn.style.background = 'rgba(0,0,0,0.9)';
                    changeBtn.onmouseleave = () => changeBtn.style.background = 'rgba(0,0,0,0.7)';
                    changeBtn.onclick = (e) => {
                        e.stopPropagation();
                        this.selectImageFile(block.id);
                    };
                    
                    const deleteImgBtn = document.createElement('button');
                    deleteImgBtn.innerHTML = '🗑 Удалить';
                    deleteImgBtn.style.cssText = 'background: rgba(0,0,0,0.7); color: white; border: none; border-radius: 8px; padding: 6px 12px; cursor: pointer; font-size: 12px; transition: all 0.2s;';
                    deleteImgBtn.onmouseenter = () => deleteImgBtn.style.background = 'rgba(0,0,0,0.9)';
                    deleteImgBtn.onmouseleave = () => deleteImgBtn.style.background = 'rgba(0,0,0,0.7)';
                    ...
                    uploadBtn.style.cssText = 'background: #3b82f6; color: white; border: none; border-radius: 12px; padding: 12px 24px; cursor: pointer; font-size: 14px; display: flex; align-items: center; gap: 8px; transition: all 0.2s;';
                    uploadBtn.onmouseenter = () => uploadBtn.style.transform = 'scale(1.02)';
...
function generateAndSave() {
    ...
    if (contentField) {
        ...
        const fullContent = css + html;
        contentField.value = fullContent;
        
        articleBuilder.showNotification('✓ HTML сгенерирован и сохранен!', 'success');
        closeArticleBuilder();
    }
}
