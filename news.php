...
if (isset($_GET['news']) && !empty($_GET['news'])) {
    $newsItem = getNewsBySlug($_GET['news']);
}

// Устанавливаем мета-данные страницы
if ($newsItem) {
    $pageTitle = htmlspecialchars($newsItem['title']) . ' - Новости - ' . getSetting('site_title');
    ...
    $pageImage = !empty($newsItem['image_path']) ? $newsItem['image_path'] : getSetting('logo_path');
} else {
    $pageTitle = 'Новости - ' . getSetting('site_title');
...
if (!$newsItem) {
    $newsList = getNews($newsPerPage, $offset);
